...
double sorozat(double elso, double masodik, int a, int b) {
    ...
    for(int i=a; i<=b; i++)
    {
        eleme = elso+((i-1)*(masodik-elso));
        szorzat = szorzat * eleme;
    }
    return szorzat;
}
...
long legzsufoltabb(Ketrec ketrecek[], int db) {
    ...
    for(int i=1; i<db; i++)
    {
        if((ketrecek[i].ketrecMeret/ketrecek[i].allatokSzama)<(ketrecek[a].ketrecMeret/ketrecek[a].allatokSzama))
            a = i;
    }
    return ketrecek[a].azonosito;
}
/***********************************************************************
************************************************************************
**
