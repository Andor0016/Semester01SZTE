...
megforditasa. A feladat megoldhato egy ciklus es ket szamlalo
hasznalataval. Az elso szamlalo a tomb elejerol, a masodik a vegerol
indul. Minden lepesben a ket szamlalo altal mutatott elemet kell
kicserelni. Figyelj arra, hogy az eredmeny-tomb meretevel terjen vissza
a fuggvenyed!
Az eredmenyt az eredeti tomb nevu tombben kell elkesziteni.
Kodold le a fuggvenyt C nyelven. A fuggveny fejlecen ne valtoztass!
*/
int tombmegforditas(int tomb[], int meret) {
    int tomb2[meret];
...
void test_1()
{
    ...
    for (i = 0; i < 2; ++i) {
        ...
        ret = tombmegforditas(tomb, testlist[i].meret);
        for(j = 0; j < ret && j < testlist[i].e_meret && tomb[j] == testlist[i].eredmeny[j]; ++j);
        if (j != ret || j != testlist[i].e_meret) {
            ...
            for(j = 0; j < ret - 1 && j < 10; fprintf(stderr, "%d,", tomb[j++]));
            if (j < 10) {
                if (j > 0) {
                    fprintf(stderr, "%d}\n", tomb[j]);
                } else {
