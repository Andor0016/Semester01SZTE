...
int compare(const void *lap1, const void *lap2) {
    LEGO_lap *lego1 = (LEGO_lap *)lap1;
    LEGO_lap *lego2 = (LEGO_lap *)lap2;

    double terulet1 = lego1->a_oldal * lego1->b_oldal;
    double terulet2 = lego2->a_oldal * lego2->b_oldal;

    if (terulet1 < terulet2) {
        return -1;
    } else if (terulet1 > terulet2) {
        return 1;
    } else {
        return 0;
    }
...
void rendez(LEGO_lap LEGO_lapok[], int n) {
    qsort(LEGO_lapok, n, sizeof(LEGO_lap), compare);
}
