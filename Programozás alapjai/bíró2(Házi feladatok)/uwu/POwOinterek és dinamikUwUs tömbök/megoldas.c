...
float* benzin(float elsoBusz, float masodikBusz, float harmadikBusz) {
    float *ertek = malloc(sizeof(float));
    *ertek = elsoBusz+masodikBusz+harmadikBusz;
    return ertek;
}
...
int utasok(Megallo* megallok, int megallokSzama, int* osszesLeszallok, int *osszesFelszallok) {
    ...
    *osszesFelszallok = 0;
    ...
    {
        *osszesFelszallok += megallok[i].felszallok;
        *osszesLeszallok += megallok[i].leszallok;
    }
    if(*osszesFelszallok == *osszesLeszallok)
        return 1;
...
int* nyereseg(int jegyek, int napiJegyek) {
    int* jegyar = malloc((jegyek+napiJegyek) * sizeof (int));
    int poz = 0;
    ...
    for(int i=0; i<napiJegyek; i++)
    {
        jegyar[poz] = NAPIJEGY_AR;
        poz++;
    }
    return jegyar;
}
...
int** csapda_atalakitas(int* csapdak, int savok, int utszakaszok) {
    int** csapdas = malloc((savok * utszakaszok) * sizeof(int));
    int poz = 0;
    for(int i=0; i<savok;i++)
    {
        for(int j=0; j<utszakaszok; j++)
        {
            csapdas[i][j] = csapdak[poz];
            poz++;
        }
    }
    return csapdas;
