...
void kalozok(int elso, int masodik, int harmadik) {
    printf("A harom legkozelebbi hajon levo kalozok: %d, %d, %d db.\n", elso, masodik, harmadik);
    printf("Az atlagos legenyseg meret: %.2f.\n", (float)(elso+masodik+harmadik)/3);
}
/*
...
float hajozas() {
    ...
    do {
        scanf("%d %d", &a, &b);
        if(a!=0 && b!=0)
            hossz += (float) sqrt(((a-pa)^2)+((b-pb)^2));
