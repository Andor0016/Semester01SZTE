...
int maximalis_rakomany_tomeg(Szan szan, float mikulas_tomeg) {
    int out = szan.maxTomeg;
    out = out - (int)(mikulas_tomeg*1000);
    if(szan.renszarvasok < 5)
        out = out/2;
...
double ossz_koltseg(double kezdeti_koltseg, double novekmeny, Idoszak idoszak) {
    double osszeg = 0;
    double kezdokiadas = kezdeti_koltseg;
    double szorzo = 1+(novekmeny/100);
    for(int i=1; i<=(idoszak.kezdet-2000); i++)
    {
        kezdokiadas = kezdokiadas * szorzo;
    }
    osszeg = kezdokiadas;
    for(int i=0; i<(idoszak.veg-idoszak.kezdet); i++)
    {
        kezdokiadas = kezdokiadas * szorzo;
        osszeg += kezdokiadas;
    }
...
float karbecsles(float karok[], int darabszam) {
    if(darabszam == 1) {
        return karok[darabszam - 1];
    }
    else
    {
        return karok[darabszam - 1] + karbecsles(karok, darabszam - 1);
    }
}
...
void test_3(){
    ...
    struct {
        ...
        int c;
            ...
            {{400, 150000, 3}, 120000, 15000},
            {{400, 160000, 5}, 120000, 40000}
    };
    for (i = 0; i < 2; ++i) {
